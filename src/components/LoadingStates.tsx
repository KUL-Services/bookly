...
export const PageLoader = ({ message }: { message?: string }) => (
  <div className='flex flex-col items-center justify-center min-h-[400px] w-full'>
    <div className='flex flex-col items-center space-y-6'>
      <BrandedSpinner size={96} />
      {message && message !== 'Loading...' && (
        <div className='space-y-2 text-center animate-pulse'>
          <p className='text-gray-600 font-medium font-mono text-sm tracking-wide uppercase'>{message}</p>
        </div>
      )}
    </div>
  </div>
          ...
          {Array.from({ length: rows }).map((_, rowIndex) => (
            <div key={rowIndex} className='flex space-x-4'>
              {Array.from({ length: columns }).map((_, colIndex) => (
                <Skeleton key={colIndex} variant='text' width={colIndex === 0 ? '40%' : '20%'} height={20} />
              ))}
            </div>
          ))}
        </div>
...
export const ButtonLoader = ({ size = 16 }: { size?: number }) => <BrandedSpinner size={size} color='inherit' />
...
export const LoadingOverlay = ({ message = 'Loading...', transparent = false }) => (
  ...
  >
    <div className='flex flex-col items-center space-y-4'>
      ...
      <p className='text-gray-600 font-medium text-sm font-mono tracking-wide'>{message}</p>
    </div>
  </div>
)
