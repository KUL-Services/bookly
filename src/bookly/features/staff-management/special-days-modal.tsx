...
export function SpecialDaysModal({ open, onClose }: SpecialDaysModalProps) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this rule?')) {
      try {
        ...
        setSnackbar({ open: true, message: error?.message || 'Failed to delete holiday hours rule', severity: 'error' })
      }
    }
  }
  ...
  const handleSave = async () => {
    ...
    const ruleToSave = {
      ...editingRule,
      shifts: editingRule.type === 'custom' && editingRule.shifts?.length ? editingRule.shifts : []
    }
    ...
    try {
      ...
      setSnackbar({ open: true, message: error?.message || 'Failed to save holiday hours rule', severity: 'error' })
    }
  }
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          {[
            ...
            >
              ...
              <Typography
                fontWeight={600}
                color={editingRule?.type === option.value ? 'text.primary' : 'text.secondary'}
              >
                {option.label}
              </Typography>
            </Paper>
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth='sm' fullWidth>
      ...
      <DialogContent sx={{ p: 3, minHeight: 400 }}>{view === 'list' ? renderList() : renderEdit()}</DialogContent>
